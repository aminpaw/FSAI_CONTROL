...
if __name__ == "__main__":
    ...
    (
        ...
        reg_smooth_opts={"k_reg": 3, "s_reg": 1},
        stepsize_opts={
            "stepsize_prep": 1.0,
            "stepsize_reg": 1.0,
    ...
    # Plot Path
    path_result = reftrack_interp[:, 0:2] + normvec_normalized_interp * np.expand_dims(
        alpha_mincurv, axis=1
    )
    bound1 = reftrack_interp[:, 0:2] - normvec_normalized_interp * np.expand_dims(
        reftrack_interp[:, 2], axis=1
    )
    bound2 = reftrack_interp[:, 0:2] + normvec_normalized_interp * np.expand_dims(
